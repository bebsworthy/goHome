...
const app = new Hono();
export async function getImagesFromDirectory(status, page, pageSize) {
    ...
    const sortedImages = imagesWithStats.sort((a, b) => b.lastModified.getTime() - a.lastModified.getTime());
...
export function getDirectoryByStatus(status) {
    switch (status) {
...
app.get('/images', async (c) => {
    try {
        const status = (c.req.query('status') || ImageStatus.PENDING);
