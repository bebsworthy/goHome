...
const fixTimeFormat = (time: string | null | undefined, def?: string | undefined): string | undefined => {
  if (!time || time.trim() === '') {
    return def;
  }
  ...
  } else { return time.trim(); }

  return def
}

const fixPhoneFormat = (phone: string | null | undefined): string | undefined => {
  if (!phone || phone.trim() === '') {
    return undefined;
  }
...
const fixEmailFormat = (email: string | null | undefined): string | undefined => {
  if (!email || email.trim() === '') {
    return undefined;
  }
...
export async function processImagesDirectory() {
  ...
  const makeImageProcessingInfo = (data: any | undefined, dbId: any | undefined, error?: any) => ({
